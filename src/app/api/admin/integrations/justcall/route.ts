...
export async function GET() {
  ...
  try {
    const history = await getJustCallSyncHistory(25);
    return NextResponse.json(createSuccessResponse(history));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to fetch JustCall sync history.";
    return NextResponse.json(createErrorResponse("JUSTCALL_HISTORY_FAILED", message), {
      status: 500,
    });
  }
}
